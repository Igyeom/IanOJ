# Overview
Welcome to the official IanOJ repository. Currently, this repo serves as a place where users can contribute problems from the KL Coding Cup to be archived on IanOJ. Right now, the biggest problem is actually transferring the problem statements to the website and getting test data for each problem. Anyone who contributes can get a Contributor badge with a tier based on how many problems they have helped archive.

~~polygon support coming soon, i promise~~

# Contributing

## Contribution Points
- The level of contribution from a user is determined by their contribution points (shortened to CP lol)
- Archiving a KL Coding Cup problem gains you **2 contribution points**, and peer-checking gains you **1 contribution point**.
- Note: creating/checking a custom problem gains you **0 contribution points**, but you can do so for fun.

## Instructions
1. Find a problem from the [KL Coding Cup resources page](https://ianoj.com/klcc) to archive.
2. Use the [problem creation page](https://ianoj.com/problem-creator) to fill in the details of the problem. Use [LaTeX](https://en.m.wikipedia.org/wiki/LaTeX) where appropriate. (inline LaTeX should be wrapped with dollar signs, like `$this$`)
3. Use the **Show Preview** button to check that the problem looks correct.
4. Press **Update JSON Data** and then copy the JSON to your clipboard.
5. Next, solve the problem (generative AI is allowed as long as it provides a 100% accurate solution, although not recommended) and create at least 50 random test cases, with pairs of files named `X.in` and `X.out`, with `X` starting from 1. Compress the folder containing these files as a ZIP file.
6. Finally, submit an issue (**not a pull request**) and attach both the problem JSON generated by the website and the ZIP file of the test cases. Use the tag `contribution`.
7. The problem needs to be checked by a mininum of 1 other user in order for the problem to be published. Confirm that the problem works as intended in the comments of the GitHub issue. Checking a problem counts as 1 contribution point.
8. When it gets approved, your problem will be on the IanOJ website and you will be credited on the problem sources. You get 2 contribution points for creating the issue.

## Credits

### Badges
- Tier 1: 1 point
- Tier 2: 10 points
- Tier 3: 50 points
- Tier 4: 100 points
- Tier 5: 200 points
- Tier 6: 500 points

### Leaderboard
A contribution leaderboard can be found [here](https://ianoj.com/contributions).
